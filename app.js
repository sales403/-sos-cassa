...
async function signupClient(){
  ...
  try{const sb=await getSupabase();const email=$('signupEmail').value.trim(),password=$('signupPassword').value,name=clampText($('signupName').value,80),phone=digits($('signupPhone').value);if(!name)throw new Error('Inserisci nome locale/richiedente.');if(phone.length<9)throw new Error('Inserisci un telefono valido.');if(!email||password.length<8)throw new Error('Email valida e password di almeno 8 caratteri.');const {data,error}=await sb.auth.signUp({email,password,options:{data:{display_name:name,phone}}});if(error)throw error;if(data.session){authSession=data.session;authUser=data.user;await loadMyProfile();await updateMyProfile({displayName:name,phone});st.className='status-line ok';st.textContent='✓ Account creato.';setTimeout(openClient,150);}else{st.className='status-line ok';st.textContent='✓ Account creato. Controlla la tua email per confermarlo, poi accedi.';}}
