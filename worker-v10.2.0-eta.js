const VERSION = 'SOS Rider API 10.2.0';
...
async function buildEtaState(env) {
  ...
  // Normally there is one delivery physically in progress. If legacy data has
  // more than one, process them deterministically before the accepted queue.
  ...
  for (let i = 0; i < acceptedRows.length; i++) {
    ...
    if (lastPoint && validCoord(lastPoint.lat, lastPoint.lon)) {
      const transferKm = estimatedRoadKm(
        lastPoint.lat,
        lastPoint.lon,
        Number(r.pickup_lat),
        Number(r.pickup_lon)
      ...
      // This is the only manual anchor left: time from Marcello's current
      // position to the first pickup. Every following leg is automatic.
      pickupMin = firstPickupEtaMin;
    }

    const deliveryMin =
      pickupMin +
      ETA_MODEL.pickupBufferMin +
      travelMinutes(r.distance_km, r.service);
  ...
  // For requests not yet accepted, show only a non-binding queue preview.
  ...
  const availability = !enabled
    ? {
        ...
        etaModel: {
          automatic: true,
          ebikeKmh: ETA_MODEL.ebikeKmh,
          windowMin: ETA_MODEL.windowMin
        },
        updatedAt: p.updated_at
      }
    : active
      ? {
          ...
          firstPickupEtaMin,
          etaMin: nextFreeMin,
          nextAvailableAt: isoPlusMinutes(nextFreeMin),
          availableEtaMin: 5,
          availableEtaMax: 10,
          etaModel: {
            automatic: true,
...
async function listRiderRequests(url, env, cors) {
  ...
  return json({
    ok: true,
    requests: (r.results || []).map(row => attachEta(row, etaState.etaByCode))
  }, 200, cors);
}
