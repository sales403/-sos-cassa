export default {
  async fetch(request, env) {
    ...
    if (url.pathname === "/") {
      return new Response("SOS Rider V7 proxy OK", { status: 200, headers: cors });
    }

    // Autocomplete indirizzi: Photon/OpenStreetMap
    ...
    if (url.pathname === "/api/route" && request.method === "POST") {
      if (!env.GOOGLE_MAPS_API_KEY) {
        return json({ error: "Manca GOOGLE_MAPS_API_KEY nei Secrets del Worker Cloudflare." }, 500, cors);
      }

      let body;
      try { body = await request.json(); }
      catch { return json({ error: "Richiesta non valida." }, 400, cors); }
      ...
      if (!origin || !destination) {
        return json({ error: "Origine e destinazione sono obbligatorie." }, 400, cors);
      }
      ...
      // Traffic aware ha senso per auto; bici/moto mantengono richiesta semplice.
      ...
      try {
        ...
        if (!r.ok) {
          const msg = data?.error?.message || "Google Routes non disponibile.";
...
function qualify(address) {
  ...
  return /\b(italia|italy)\b/i.test(a) ? a : `${a}, Modena, Italia`;
}
